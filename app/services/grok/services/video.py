...
def _resolve_seed_default(
    *,
    ...
    if default_key not in video_defaults:
        return current_value
    return video_defaults[default_key]
...
class VideoService:
    ...
    async def completions(
        ...
        video_config_fields_set: Optional[set[str]] = None,
    ):
        ...
        provided_fields = set(video_config_fields_set or set())
        resolved_seed = await VideoSeedService.resolve_seed(video_seed)
        if isinstance(video_seed, dict) and video_seed and not resolved_seed:
            raise ValidationException(
                message="video_seed is invalid, expired, or unavailable",
                param="video_config.video_seed",
                code="invalid_video_seed",
            )
        seed_defaults = (
            resolved_seed.get("video_defaults", {})
            if isinstance(resolved_seed, dict)
            else {}
        )
        aspect_ratio = _resolve_seed_default(
            field_name="aspect_ratio",
            current_value=aspect_ratio,
            provided_fields=provided_fields,
            video_defaults=seed_defaults,
            default_key="aspectRatio",
        )
        video_length = int(
            _resolve_seed_default(
                field_name="video_length",
                current_value=video_length,
                provided_fields=provided_fields,
                video_defaults=seed_defaults,
                default_key="videoLength",
            )
            or 6
        )
        resolution = str(
            _resolve_seed_default(
                field_name="resolution_name",
                current_value=resolution,
                provided_fields=provided_fields,
                video_defaults=seed_defaults,
                default_key="resolutionName",
            )
            or "480p"
        )
        preset = str(
        ...
        seed_post_id = ""
        if resolved_seed:
            seed_post_id = _pick_str(resolved_seed.get("parent_post_id"))
            image_url = _pick_str(resolved_seed.get("image_url_upstream"))
            if image_attachments:
                logger.info(
                    "video_seed provided; ignoring inline image attachments in favor of persisted seed"
                )
        ...
        if seed_post_id:
            logger.info(f"Video generation using persisted seed post: {seed_post_id}")
        elif image_url:
