...
| └─`video_seed` | object | 图生视频种子 | 可传 `seed_id`，或兼容传 `parent_post_id` / `image_url_upstream` |
...
- `grok-imagine-1.0-video` 也支持通过 `video_config.video_seed` 复用图片生成阶段返回的种子；若同时提供 `video_seed` 与 `image_url`，优先使用 `video_seed`。
- 除上述外的其他参数将自动丢弃并忽略。
...
- 非流式图片返回的每个 `data` 项会附带 `video_seed`；可在后续 `grok-imagine-1.0-video` 请求里通过 `video_config.video_seed.seed_id` 复用该种子。
